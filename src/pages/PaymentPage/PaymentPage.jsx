...
import { createOrder, updateOrderPaid } from '../../userService/OrderServices';
...
const PaymentPage = () => {
    ...
    const tax = 0; // Example static tax, replace with dynamic calculation if needed
    const shipping = deliveryMethod === 'express' ? 32000 : 16000; // Dynamic shipping based on delivery method
            ...
            try {
                ...
                if (response.success) {
                    if (paymentMethod === 'cash') {
                        message.success('Đơn hàng của bạn đã được tạo thành công. Bạn sẽ thanh toán khi nhận hàng.', 5);
                        navigate('/my-order/:id');
                    } else {
                        // Redirect to the payment page for other payment methods
                        window.location.href = response.url;
                    }
                    message.success('Đơn hàng của bạn đã được tạo thành công. Vui lòng kiểm tra email để biết thêm chi tiết.', 5);
                } else {
                    message.error(response.message || 'Đã xảy ra lỗi, vui lòng thử lại.', 5);
                }
                ...
                <Row className='payment_wrapper'>
                    ...
                    <Col className="col_item" span={6}>
                        ...
                        <div className="payment_method">
                            ...
                            > <div>
                                    <Radio value='standard'>Giao hàng tiêu chuẩn (16,000₫)</Radio>
                                </div>
                                <div>
                                    <Radio value='express'>Giao hàng nhanh (32,000₫)</Radio>
                                </div>
                            </Radio.Group>
                        </div>
                        <Divider />
